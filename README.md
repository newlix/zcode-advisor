...
| Review gate (`Stop` hook) | One wake per session when files were edited but `review_change` was never called: the turn is extended once with a reminder (Stop + `continue:true` — the only channel whose text actually reaches the model, since ZCode drops `additionalContext` on Stop); **makes no API call**; trivial changes (docs/formatting/one-liners) may just finish their reply | Rules |
